...
class OrderController extends Controller
{
    ...
    public function status(Request $request)
    {
        ...
        else if($request->order_status == 'refunded')
        {
            ...
            {
                Toastr::warning(__('you can not refund a cod order'));
                return back();
            }
        ...
        else if($request->order_status == 'canceled')
        {
            ...
            {
                Toastr::warning(__('You can not cancel a completed order'));
                return back(); 
            }
            if($order->delivery_man)
            {
    ...
    public function add_delivery_man($order_id, $delivery_man_id)
    {
        ...
        if($order->delivery_man_id == $delivery_man_id)
        {
            return response()->json([
                'errors'=>[
                    ['delivery_man_id'=> __('order already assign to this deliveryman')]
                ]
            ], 400);
        }
        if($deliveryman)
        {
            if($deliveryman->current_orders >= config('dm_maximum_orders'))
            {
                return response()->json([
                    'errors'=>[
                        ['current_orders'=> __('Dm maximum order exceed warning')]
                    ]
                ], 404);
            }
            if($order->delivery_man)
            {
                ...
                $data = [
                    ...
                    'description' => __('You are unassigned from a order'),
                    'order_id' => '',
                    'image' => '',
                    'type'=> 'assign'
                ];
    ...
    public function update_shipping(Request $request,Order $order)
    {
        ...
        $order->save();
        Toastr::success(__('delivery address updated'));
        return back();
    }
    ...
    public function add_payment_ref_code(Request $request, $id)
    {
        ...
        Toastr::success(__('payment reference code is added'));
        return back();
    }
    ...
    public function update(Request $request, Order $order)
    {
        ...
        if(!$request->session()->has('order_cart'))
        {
            Toastr::error(__('order data not found'));
            return back();
        }
        $cart = $request->session()->get('order_cart', collect([]));
            ...
            if($c['status'] == true)
            {
                ...
                if ($c['item_campaign_id'] != null) 
                {
                    ...
                    if ($product) {
                        ...
                        Toastr::error(__('food not found'));
                        return back();
                    }
                } else {
        ...
        {
            Toastr::error(__('you need to order at least', ['amount'=>$restaurant->minimum_order.' '.Helpers::currency_code()]));
            return back();
        }
