            ...
            <div class="rest-info-wrap">
                ...
                <div>
                    ...
                    <div class="details justify-center">
                        ...
                        @endif
                        <div class="detail-block">
                            <h5 class="pt-2">{{ $restaurant->delivery_time }}</h5>
...
<script>
    ...
    function removeFromCart(key) {
        $.post('{{ route('remove-from-cart') }}', {_token: '{{ csrf_token() }}', key: key}, function (data) {
            if (data.errors) {
    ...
    function emptyCart() {
        $.post('{{ route('emptyCart') }}', {_token: '{{ csrf_token() }}'}, function (data) {
            updateCart();
   ...
   $(function(){
    ...
    function updateQuantity(e){
        ...
        if (valueCurrent >= minValue) {
            $.post('{{ route('updateQuantity') }}', {_token: '{{ csrf_token() }}', key: key, quantity:valueCurrent}, function (data) {
                updateCart();
            });
        } else {
            Swal.fire({
