    ...
    <div class="page-header">
        ...
        <div class="js-nav-scroller hs-nav-scroller-horizontal">
            ...
            <li class="nav-item">
                ...
                <li class="nav-item">
                    <a class="nav-link active" href="{{route('admin.vendor.view', ['restaurant'=>$restaurant->id, 'tab'=> 'discount'])}}"  aria-disabled="true">{{__('Discount')}}</a>
                </li>
                <li class="nav-item">
                            ...
                            <div class="row">
                                <div class="col-6 border-right">
                                    <ul class="list-unstyled list-unstyled-py-3 text-dark mb-3">
                                        <li class="pb-1 pt-1">{{__('Start date')}} : {{$restaurant->discount?date('Y-m-d',strtotime($restaurant->discount->start_date)):''}}</li>
                                        <li class="pb-1 pt-1">{{__('End date')}} : {{$restaurant->discount?date('Y-m-d', strtotime($restaurant->discount->end_date)):''}}</li>
                                        <li class="pb-1 pt-1">{{__('Start time')}} : {{$restaurant->discount?date(config('timeformat'), strtotime($restaurant->discount->start_time)):''}}</li>
                                        <li class="pb-1 pt-1">{{__('End time')}} : {{$restaurant->discount?date(config('timeformat'), strtotime($restaurant->discount->end_time)):''}}</li>
                                    </ul>
                                </div>
                                <div class="col-6">
                                    <ul class="list-unstyled list-unstyled-py-3 text-dark mb-3">
                                        <li class="pb-1 pt-1">{{__('Discount')}} : {{$restaurant->discount?round($restaurant->discount->discount):0}}%</li>
                                        <li class="pb-1 pt-1">{{__('max discount')}} : {{\App\CentralLogics\Helpers::format_currency($restaurant->discount?$restaurant->discount->max_discount:0)}}</li>
                                        <li class="pb-1 pt-1">{{__('min purchase')}} : {{\App\CentralLogics\Helpers::format_currency($restaurant->discount?$restaurant->discount->min_purchase:0)}}</li>
                                    </ul>
                                </div>
                            </div>
                            @else
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalCenterTitle">{{$restaurant->discount?__('Update'):__('add discount')}}</h5>
        ...
        <form action="{{route('admin.vendor.discount',[$restaurant['id']])}}" method="post" id="discount-form">
            ...
            <div class="row">
                <div class="col-md-4 col-6">
                    <div class="form-group">
                        <label class="input-label" for="title">{{__('Discount')}} (%)</label>
                ...
                <div class="col-md-4 col-6">
                    <div class="form-group">
                        <label class="input-label" for="title">{{__('min purchase')}} ({{\App\CentralLogics\Helpers::currency_symbol()}})</label>
                ...
                <div class="col-md-4 col-6">
                    <div class="form-group">
                        <label class="input-label" for="title">{{__('max discount')}} ({{\App\CentralLogics\Helpers::currency_symbol()}})</label>
            ...
            <div class="form-group text-center">
                <button type="submit" class="btn btn-primary">{{$restaurant->discount?__('Update'):__('Add')}}</button>
                @if($restaurant->discount)
                <button type="button" onclick="form_alert('discount-{{$restaurant->id}}','Want to remove discount?')" class="btn btn-danger">{{__('Delete')}}</button>
                @endif
            </div>
        </form>
